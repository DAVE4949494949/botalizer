...
//        pre($json, 1);
    ...
    static public function RunParseActionEN()
    {
        ...
        $html = Template::RenderTemplate('languages/en/analiz/analiz.twig', (array)Repository::$data);
        return new Response($html);
    }
...
//            Repository::$data['url'] = Url::$url;
    ...
    static public function UpdateCategoriesAction($url)
    {
        Url::$url = $url;
        $url = strtolower(Url::getUrl());
        ...
        if (Analiz::CheckSite($url)) {
            $taskId = Analiz::getTaskId($url);
            $html = Template::RenderTemplate('update-categories.twig', array
            ('taskId' => $taskId));
            return new Response($html);
        }
...
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        $html = Template::RenderTemplate('languages/en/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionFR($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/fr/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/fr/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionDE($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/de/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/de/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionES($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/es/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/es/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionIT($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/it/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/it/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionNL($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/nl/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/nl/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionJA($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/ja/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/ja/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionTR($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/tr/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/tr/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionPL($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/pl/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/pl/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionRO($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/ro/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/ro/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionZH($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/zh/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/zh/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionKO($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/ko/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/ko/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionPT($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/pt/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/pt/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionID($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/id/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/id/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionBR($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/br/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/br/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionRU($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/ru/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/ru/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }

    public static function ResultActionHI($url)
    {
//        ini_set('display_errors', 1);
//        error_reporting(E_ALL);
        $data = \Userset\Analiz\Models\Analiz::Get($url);
        if ($data == FALSE) {
            return new RedirectResponse(Repository::$config['base_url'] . '/hi/');
        }
        $data->similar = array();
//        if(isset($_GET['testim']))
//            pre(self::get_domain_name($data->url), 1);
//        $data->siteName = substr($data->url, 0, strpos($data->url, '.'));
        ...
        if ($similar = \Userset\Analiz\Models\Analiz::GetSimilar($data))
            $data->similar = $similar;
        $html = Template::RenderTemplate('languages/hi/analiz/analiz.twig', (array)$data);
        return new Response($html);
    }
